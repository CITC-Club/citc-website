...
import { Pencil, Trash2, Search, Calendar, MapPin, ChevronDown } from "lucide-react";
...
export default function EventsTable({ events }: Props) {
  ...
  const statusColors: Record<string, string> = {
    running: "bg-red-50 text-red-700 border-red-100 dark:bg-red-950/20 dark:text-red-450 dark:border-red-900/25",
    upcoming: "bg-cyan-50 text-cyan-700 border-cyan-100 dark:bg-cyan-950/20 dark:text-cyan-400 dark:border-cyan-900/25",
    ...
    <div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/60">
              <th className="text-left px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Event</th>
              <th className="text-left px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Date &amp; Time</th>
              <th className="text-left px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Location</th>
              <th className="text-left px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Status</th>
            ...
            {filtered.map((event) => (
              ...
              >
                      ...
                      {event.tags && event.tags.length > 0 && (
                        <div className="flex gap-1 mt-1.5">
                          {event.tags.slice(0, 2).map((tag) => (
                            <span key={tag} className="px-1.5 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-cyan-50 dark:bg-cyan-950/40 text-cyan-600 dark:text-cyan-400 border border-cyan-100/50 dark:border-cyan-900/20">
                              {tag}
                            </span>
                          ))}
                        </div>
                ...
                <td className="px-6 py-4.5">
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 pl-6">{event.time}</p>
                </td>
                <td className="px-6 py-4.5">
                  <div className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                ...
                <td className="px-6 py-4.5">
                  <span className={`inline-flex px-2.5 py-0.5 rounded-lg text-xs font-semibold border capitalize ${statusColors[event.status] || ""}`}>
                    {event.status}
                  </span>
                </td>
                  ...
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    {confirmDelete === event.id ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={() => setConfirmDelete(null)} className="px-2.5 py-1 text-xs font-bold text-slate-650 bg-slate-100 hover:bg-slate-200 dark:text-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-lg transition-colors cursor-pointer">
                          Cancel
                        </button>
                      </div>
                    ) : (
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 bg-slate-50/10 dark:bg-slate-900/5">
          <p className="text-slate-400 dark:text-slate-600 text-sm">No events match your active filters</p>
        </div>
      )}
